...
* Также для маленькой модели так сложилось, что качество для запусков с 4 ddp процессами, размером батча 64 и с 2 ddp процессами, размером батча 128 ведет себя одинаково, то же самое верно для пары запусков с 4 ddp процессами, размером батча 128 и с 2 ddp процессами, размером батча 256.
...
Использовать ddp стоит при достаточно большом размере модели, иначе мы не получим выигрыша во времени. Также стоит учитывать, что при достаточно большом количестве ddp процессов, аналогично как с размером батча, можно так же потерять в качестве.
